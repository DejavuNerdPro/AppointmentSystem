...
export class PopupModalComponent implements OnInit, OnChanges {
  ...
  constructor(
    ...
    this.refreshApi.listenAttendee().subscribe(
      (refreshData)=>{
        console.log("Attendee : ",refreshData);
        console.log("Refreshed Attendee : ",refreshData);
        this.attendees=refreshData;
    ...
    this.refreshApi.listenCurrentCalendar().subscribe(
      (refreshData)=>{
        console.log("File Host : ",refreshData);
        console.log("Refreshed File Host : ",refreshData);
        this.fileIdHost=refreshData;
        console.log("Refreshed File Host : ",this.attendees);
      }
    );
  }
  ...
  deletion() {
    ...
    Swal.fire({
      ...
      .subscribe((data) => {
        console.log("Delete Schedule Message : ", data);
  ...
  saveFiles() {
    this.scheduleService.addFile(this.files, this.title).subscribe(
      ...
      (result) => {
        this.refreshApi.refreshDataCurrentCalendar(this.currentLoginUserId);
        this.refreshApi.refreshDataSearchCalendar(this.userId);
        this.refreshApi.refreshFileHost(this.scheduleId);
        if (result != null) {
