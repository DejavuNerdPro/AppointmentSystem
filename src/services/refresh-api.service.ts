...
export class RefreshApiService {
  ...
  private _fileHostListener=new Subject();
  private _profileListener=new Subject();
  eventarray:any;
  ...
  listenFileHost():Observable<any>{
    return this._fileHostListener.asObservable();
  }
  listenProfile():Observable<any>{
    return this._profileListener.asObservable();
  }
  ...
  async refreshFileHost(scheduleId:any){
    let result:any=await firstValueFrom(this.httpService.getMethod("http://localhost:8081/file/getAllScheduleFiles?scheduleId=" +scheduleId));
    return this._fileHostListener.next(result);
  }

  async refreshProfile(currentUserID:any){
    let result:any=await firstValueFrom(this.httpService.getMethod("http://localhost:8081/image/getImage?id=" + currentUserID));
    let profile:any = this.sanitizer.bypassSecurityTrustUrl(
      "data:image/png;base64," + result.userImageData
    );
    return this._profileListener.next(profile);
  }
